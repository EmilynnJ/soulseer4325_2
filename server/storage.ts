...
export interface IStorage {
  ...
  getUserByUsername(username: string): Promise<User | undefined>; // Added
  ...
  updateReading(id: number, reading: Partial<typeof readings.$inferInsert>): Promise<Reading | undefined>; // Reading ID is serial
...
export class DatabaseStorage implements IStorage {
  ...
  async createUser(user: InsertUser): Promise<User> {
    try {
      ...
      const userToCreate: typeof users.$inferInsert = {
        ...user, // Spread all fields from InsertUser
        email: user.email, // Explicitly ensure email is passed
        role: user.role,   // Explicitly ensure role is passed (it has a DB default, but InsertUser requires it)
        createdAt: now,
  ...
  async updateUser(id: string, userData: Partial<typeof users.$inferInsert>): Promise<User | undefined> {
    try {
      // Ensure updatedAt is handled by the database $onUpdate or manually if needed
      const dataToUpdate = { ...userData, updatedAt: new Date() }; // Manually set updatedAt
      if (Object.keys(dataToUpdate).length === 1 && 'updatedAt' in dataToUpdate) { // Avoid empty update if only id is passed indirectly
  ...
  async updateReading(id: number, readingData: Partial<typeof readings.$inferInsert>): Promise<Reading | undefined> {
    try {
      const dataToSet = { ...readingData };
      // Drizzle's .set() expects fields defined in the table.
      // Ensure any transformations or defaults are handled if $inferInsert differs from table structure for updates.
  ...
  async updateProduct(id: number, productData: Partial<typeof products.$inferInsert>): Promise<Product | undefined> {
    try {
      const [updatedProduct] = await db.update(products)
        .set(productData) // Assuming $inferInsert is suitable for updates directly
  ...
  async updateOrder(id: number, orderData: Partial<typeof orders.$inferInsert>): Promise<Order | undefined> {
    try {
      const [updatedOrder] = await db.update(orders)
        .set(orderData) // Assuming $inferInsert is suitable for updates directly
  ...
  async updateLivestream(id: number, livestreamData: Partial<typeof livestreams.$inferInsert>): Promise<Livestream | undefined> {
    try {
      const [updatedLivestream] = await db.update(livestreams)
        .set(livestreamData) // Assuming $inferInsert is suitable for updates directly
  ...
  async updateForumPost(id: number, forumPostData: Partial<typeof forumPosts.$inferInsert>): Promise<ForumPost | undefined> {
    try {
      const [updatedForumPost] = await db.update(forumPosts)
        .set({ ...forumPostData, updatedAt: new Date() }) // Manually set updatedAt
